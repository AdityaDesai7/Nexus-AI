...
# trading_bot/ui/institutional_analysis.py
import os
import logging
from datetime import datetime, timedelta
import streamlit as st
...
def normalize_response(resp):
    """
    Convert agent runner responses into a normalized dict for UI consumption.
    Accepts:
     - dict (returns shallow copy)
     - object with .dict()
...
def display_json_friendly(label: str, payload):
    """Display dict/df/list/string safely in Streamlit."""
    st.markdown(f"### {label}")
    if payload is None:
        st.info("No output")
        return
    if isinstance(payload, dict):
        # don't mutate caller payload
        p = dict(payload)
        df = p.pop("df", None)
        try:
            st.json(p)
...
def safe_get_latest_close(payload):
    """Try multiple shapes to get latest close price."""
    try:
        if payload is None:
            return None
        if isinstance(payload, dict):
...
# -------------------------
# Runner + wrapper factory
# -------------------------
def ensure_inst_runner():
    """
    Create an AgentRunner + institutional wrappers and register them.
    Stored in st.session_state['inst_runner'].
    """
    if "inst_runner" not in st.session_state:
        runner = AgentRunner()
        # instantiate wrappers (expects create_inst_wrappers exists)
        ...
        st.session_state.inst_wrapped = list(wrapped.keys())
        logger.info("Institutional AgentRunner initialized with agents: %s", st.session_state.inst_wrapped)
    return st.session_state.inst_runner


# -------------------------
# Streamlit page
# -------------------------
def show_institutional_page():
    st.set_page_config(layout="wide", page_title="Institutional — SMART-MARKET")
    st.title("🏦 Institutional Agents — SMART-MARKET (Standalone)")

    runner = ensure_inst_runner()

    with st.sidebar:
        st.header("Run settings (Institutional)")
        ticker = st.text_input("Ticker (e.g. TCS.NS)", value=st.session_state.get("inst_ticker", "TCS.NS"))
        ...
        run_btn = st.button("🚀 Run Institutional Analysis")
        st.markdown("---")
        st.markdown("Registered institutional agents:")
        st.write(", ".join(st.session_state.get("inst_wrapped", [])))
    ...
    if not run_btn:
        st.info("Configure settings in the sidebar and click 'Run Institutional Analysis'.")
        return
    ...
    try:
        ...
        try:
            if "fetch_price" in TOOLS:
                # your toolbox may return a raw DataFrame (tool_fetch_price)
                try:
                    price_df = TOOLS["fetch_price"](ticker, start_date, end_date)
                except Exception:
                    # older tool signature might return dict {'status':'OK','df': df}
                    res = TOOLS["fetch_price"](ticker, start_date, end_date)
                    if isinstance(res, dict) and res.get("df") is not None:
                        price_df = res.get("df")
            else:
                # fallback
                price_df = fetch_data(ticker, start_date, end_date)
        except Exception as e:
        ...
        if isinstance(price_df, pd.DataFrame) and len(price_df) > 0:
            latest_close = float(price_df["Close"].iloc[-1]) if "Close" in price_df.columns else None
        else:
            latest_close = None

        progress.progress(15)
        status.info("2/6 — Running institutional agents")
        wrapped_names = st.session_state.get("inst_wrapped", [])
        results = {}
        i = 0
        total = len(wrapped_names) if wrapped_names else 1

        for name in wrapped_names:
            i += 1
            status.info(f"Running {name} ({i}/{total})")
            progress.progress(15 + int((i / total) * 60))

            # Prepare sensible input for institutional agents
            agent_input = {
                "ticker": ticker,
                "start": start_date,
                "end": end_date,
            ...
            # run safely
            out = runner.run(name, agent_input)
            norm = normalize_response(out)
            results[name] = norm

        progress.progress(80)
        status.info("3/6 — Looking for aggregator / consensus agent")

        # Try to locate aggregator result among registered agents
        aggregator_names = [k for k in wrapped_names if any(x in k.lower() for x in ("aggreg", "master", "inst", "institution"))]
        aggregator_payload = None
        aggregator_key = None
        for k in aggregator_names:
            try:
                payload = results.get(k)
                if payload and isinstance(payload, dict):
                    # check for 'result' or 'master' or dict content indicating aggregator
                    if "result" in payload or "master" in payload or "final_score" in (payload.get("result") or {}):
                        aggregator_payload = payload
                        aggregator_key = k
                        break
                    # fallback: if this agent returns 'final_score' or 'recommendation' at top-level
                    top = payload.get("result") or payload.get("master") or payload
                    if isinstance(top, dict) and any(x in top.keys() for x in ("final_score", "recommendation", "confidence")):
                        aggregator_payload = payload
                        aggregator_key = k
                        break
            except Exception:
        ...
        # If no aggregator found, try to compose a tiny consensus locally
        consensus = None
        if aggregator_payload is None:
            try:
                # quick consensus: count BUY/SELL in agent payloads if they include 'action' or 'recommendation'
                buy = 0
                sell = 0
                neutral = 0
                for n, payload in results.items():
                    try:
                        p = payload.get("result") or payload.get("master") or payload
                        if isinstance(p, dict):
                            action = p.get("action") or p.get("recommendation") or p.get("recommend") or p.get("signal")
                            if isinstance(action, str):
                                a = action.upper()
                                if "BUY" in a or "ACCUM" in a or "BULL" in a:
                                    buy += 1
                                elif "SELL" in a or "BEAR" in a or "DIST" in a:
                                    sell += 1
                                else:
                                    neutral += 1
                            else:
                        ...
                        neutral += 1
                consensus = {"buy": buy, "sell": sell, "neutral": neutral, "agents": len(results)}
            except Exception:
                consensus = None
        ...
        # ---------- DISPLAY ----------
        tab_all, tab_agg, tab_data = st.tabs(["All Institutional Agents", "Aggregator / Consensus", "Raw Price Data"])

        with tab_all:
            st.write("Outputs from institutional agents (shallow-normalized).")
            for name, payload in results.items():
                display_json_friendly(name, payload)
                st.markdown("---")

        with tab_agg:
            if aggregator_payload:
                st.markdown(f"**Aggregator found:** `{aggregator_key}`")
                display_json_friendly("Aggregator Output", aggregator_payload)
            elif consensus:
                st.markdown("**Quick consensus (local comp):**")
                st.json(consensus)
                st.markdown("Agent-by-agent quick signals (best-effort)")
                rows = []
                for n, p in results.items():
                    top = p.get("result") or p.get("master") or p
                    action = None
                    if isinstance(top, dict):
                        action = top.get("action") or top.get("recommendation") or top.get("signal")
                    rows.append({"agent": n, "action": action})
                st.table(pd.DataFrame(rows))
            else:
                st.info("No aggregator or consensus available.")

        with tab_data:
            if isinstance(price_df, pd.DataFrame) and len(price_df) > 0:
                st.markdown("**Price data (preview)**")
                st.dataframe(price_df.head(20))
            else:
                st.info("No price data available for this ticker / date range.")

        progress.progress(100)
        status.success("✅ Institutional analysis complete")

    except Exception as e:
        logger.exception("Institutional pipeline failed")
        st.error(f"Institutional pipeline error: {e}")
