...
# ======================================================
# 9 INSTITUTIONAL AGENTS (REAL DATA MODEL)
# ======================================================
...
class FFIMomentumAgent:
    ...
    def analyze(self, fii_data: Dict, price_data: pd.Series) -> AgentSignal:
        try:
            if not fii_data or price_data is None:
                return AgentSignal(self.name, 0, 20, "NEUTRAL", "No FII data")

            today = fii_data.get("today", 0)
            avg = fii_data.get("30_day_avg", 1)

            ratio = today / avg if avg != 0 else 1
            ratio = np.clip(ratio, 0.3, 2.5)

            if ratio > 2.0:
                return AgentSignal(self.name, 40, 75, "BUY", "Strong FII inflow", {"ratio": ratio})
            if ratio > 1.5:
                return AgentSignal(self.name, 20, 60, "SLIGHT_BUY", "Moderate FII inflow", {"ratio": ratio})
            if ratio < 0.5:
                return AgentSignal(self.name, -40, 75, "SELL", "Heavy FII selling", {"ratio": ratio})

            return AgentSignal(self.name, 0, 40, "NEUTRAL", "Stable FII flow", {"ratio": ratio})
...
class ExecutionBreakdownDetector:
    ...
    def analyze(self, order_data, price_data):
        try:
            if not order_data:
                return AgentSignal(self.name, 0, 25, "NEUTRAL", "No bulk/block orders")

            recent = order_data[-50:]

            buys = [o for o in recent if o.get("direction") == "BUY"]
            sells = [o for o in recent if o.get("direction") == "SELL"]

            if len(buys) > len(sells) * 1.5:
                return AgentSignal(self.name, 35, 70, "BUY", "Strong buy execution")
            if len(sells) > len(buys) * 1.5:
                return AgentSignal(self.name, -35, 70, "SELL", "Strong sell execution")

            return AgentSignal(self.name, 0, 45, "NEUTRAL", "Balanced flow")
...
class VolumePatternRecognition:
    ...
    def analyze(self, volume, price):
        try:
            if len(volume) < 10:
                return AgentSignal(self.name, 0, 20, "NEUTRAL", "Not enough volume data")

            r = volume.tail(5).mean() / volume.tail(20).mean()

            if r > 1.5:
                return AgentSignal(self.name, 35, 65, "BUY", "Volume spike", {"ratio": r})

            if r < 0.7:
                return AgentSignal(self.name, -25, 60, "SELL", "Volume dropping", {"ratio": r})

            return AgentSignal(self.name, 0, 40, "NEUTRAL", "Normal volume")
...
class IFICalculator:
    ...
    def analyze(self, volume, price):
        try:
            if len(volume) < 5:
                return AgentSignal(self.name, 0, 20, "NEUTRAL", "Not enough data")

            ifi = volume.iloc[-1] / volume.tail(30).mean()

            if ifi > 2.2:
                return AgentSignal(self.name, 50, 80, "BUY", "Strong IFI", {"ifi": ifi})
            if ifi < 0.5:
                return AgentSignal(self.name, -40, 75, "SELL", "Weak IFI", {"ifi": ifi})

            return AgentSignal(self.name, 0, 35, "NEUTRAL", "Normal IFI", {"ifi": ifi})
...
class AccumulationDetector:
    ...
    def analyze(self, price, volume):
        try:
            ...
            r = price.iloc[-1] / price.tail(20).min()

            if r < 1.03:
                return AgentSignal(self.name, 40, 70, "BUY", "Possible accumulation", {"ratio": r})

            return AgentSignal(self.name, 0, 40, "NORMAL", "No accumulation", {"ratio": r})
...
class LiquidityDetector:
    ...
    def analyze(self, volume):
        try:
            ...
            r = volume.tail(5).mean() / volume.tail(30).mean()

            if r < 0.7:
                return AgentSignal(self.name, -20, 60, "LOW_LIQ", "Low liquidity")

            if r > 1.4:
                return AgentSignal(self.name, 20, 65, "HIGH_LIQ", "High liquidity")

            return AgentSignal(self.name, 0, 45, "NORMAL", "Normal liquidity")
...
class SmartMoneyTracker:
    ...
    def analyze(self, price, volume):
        try:
            trend = price.iloc[-1] / price.iloc[-10]

            if trend > 1.03:
                return AgentSignal(self.name, 40, 70, "BUY", "Uptrend detected")

            if trend < 0.97:
                return AgentSignal(self.name, -40, 70, "SELL", "Downtrend detected")

            return AgentSignal(self.name, 0, 45, "NEUTRAL", "Flat")
...
class BlockOrderTracker:
    ...
    def analyze(self, orders):
        try:
            blocks = [o for o in orders if o.get("quantity", 0) > 50000]

            if not blocks:
                return AgentSignal(self.name, 0, 25, "NEUTRAL", "No blocks")

            buys = [o for o in blocks if o.get("direction") == "BUY"]
            sells = [o for o in blocks if o.get("direction") == "SELL"]

            if len(buys) > len(sells):
                return AgentSignal(self.name, 40, 70, "BUY", "Institutional buy")

            if len(sells) > len(buys):
                return AgentSignal(self.name, -40, 70, "SELL", "Institutional sell")

            return AgentSignal(self.name, 0, 50, "NEUTRAL", "Mixed")
...
class BreakoutDetector:
    ...
    def analyze(self, price, volume):
        try:
            high = price.tail(30).max()
            last = price.iloc[-1]

            if last >= high * 0.98:
                return AgentSignal(self.name, 55, 80, "BREAKOUT_UP", "Testing resistance")

            return AgentSignal(self.name, 0, 35, "NO_BREAKOUT", "No breakout")
...
class MasterInstitutionalAggregator:
    ...
    def __init__(self):
        self.agents = {
            ...
            "accumulation": AccumulationDetector(),
            "liquidity": LiquidityDetector(),
            "smart": SmartMoneyTracker(),
            "blocks": BlockOrderTracker(),
            "breakout": BreakoutDetector(),
        }

    def aggregate(self, fii, orders, price, volume):

        results = {}

        # run each agent safely
        for n, ag in self.agents.items():
            try:
                if n == "fii":
                    results[n] = ag.analyze(fii, price)
                elif n == "execution":
                    results[n] = ag.analyze(orders, price)
                elif n == "blocks":
                    results[n] = ag.analyze(orders)
                elif n in ("liquidity",):
                    results[n] = ag.analyze(volume)
                else:
                    results[n] = ag.analyze(price, volume)
            except Exception as e:
                results[n] = AgentSignal(n, 0, 0, "ERROR", str(e))

        # calculate consensus
        buys = [s for s in results.values() if "BUY" in s.action]
        sells = [s for s in results.values() if "SELL" in s.action]
        ...
        return AggregatedSignal(
            final_score=np.clip(50 + (len(buys) - len(sells)) * 4, 0, 100),
            recommendation=rec,
            confidence=conf,
            agent_votes={n: s.score for n, s in results.items()},
            reasoning=f"Buy={len(buys)} Sell={len(sells)}",
